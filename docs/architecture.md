...
**Several layers stop duplicate actions.** (1) `POST /checkout` takes an `Idempotency-Key`. The case this is actually built for, a client retrying after a timeout, is handled cleanly: the key is looked up first, and if it's already been recorded, the original response comes back with no new order and no new charge. Truly concurrent requests carrying the same key are a real, narrower gap: both can pass that lookup before either has saved its key row, so both proceed through checkout and payment, and only the final save collides on the key's unique constraint, by which point any duplicate order or charge has already happened. Closing that fully would mean writing the key row before the reservation attempt instead of after, which changes the shape of the transaction enough that it's tracked as a known limitation rather than quietly assumed away. (2) `payment.order_id` has its own unique constraint, so even if two requests somehow both got past the idempotency check, only one of them could ever insert a payment row for that order. (3) Webhooks are fingerprinted and checked against `processed_webhook`, so the same webhook is never processed twice. (4) The state machine simply does nothing if asked to repeat a transition it's already made. Put together: retries are fully safe, no double charge, no double shipment, no overselling; genuinely concurrent duplicate submissions are the one narrower gap, described above.
...
- *The reservation expires, and then the payment succeeds anyway* (a slow 3-DS check, for example): once the order is marked `PAID`, committing the reservation is attempted, the same step that runs for the normal case. If the reservation is still there, it converts to a real stock deduction and everything continues as normal. If it's gone, the current behavior is a deliberately conservative one: refund the customer in full and move the order to `REFUNDED`, rather than attempt a fresh reservation and risk shipping against stock that was already promised to someone else. The customer is never left having paid for something that can't actually be shipped, but a real re-reservation attempt (worth trying before falling back to a refund) is a known gap, not yet built: it would need the order's line items, which live in a different module than where this check runs today.
