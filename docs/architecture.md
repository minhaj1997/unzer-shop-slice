...
- **[customers]** `customer(id, email, password_hash, role, created_at)`, with roles `CUSTOMER` or `ADMIN`; `address(id, customer_id, label [BILLING|SHIPPING], line1, line2, city, postal_code, country, is_default)`, since a customer needs more than one address and checkout needs a clear default to pre-fill. Passwords are hashed, never stored or logged in any reversible form. A guest checkout creates an order with the contact details saved directly on it, with no account needed. If a guest later registers using the same email, past guest orders get linked to the new account by that email match at registration time, rather than asking the customer to somehow prove they were the same person.
- **[catalog]** `product(id, name, description jsonb, category_id)`, `variant(id, product_id, sku unique, attributes jsonb, price_minor bigint, currency char(3))`. **Money is always stored as a whole number in the smallest unit (like cents) plus a currency code**, never as a floating-point number.
...
- **[cart]** `cart(id, customer_id nullable, token)` for guests, `cart_item(cart_id, variant_id, qty)`. The cart does not store prices. Prices are looked up fresh each time the cart is shown, and only get locked in once checkout creates the order.
...
- *A duplicate or re-sent webhook:* it's caught and dropped by the fingerprint check. Even if it wasn't, it would be harmless, because the state machine ignores a transition it's already made.
- *Unzer is completely down when the customer tries to check out:* starting the payment fails quickly, with short timeouts (only safe, repeatable GET requests are retried, and with a backoff delay). The order stays `AWAITING_PAYMENT`, and the stock stays reserved until the reservation's time limit runs out. The customer can try paying again on that same order.
...
**What I chose, and what it costs me.** A modular monolith instead of microservices: simpler to run today, and still easy to split apart later. The cost is that everything shares one blast radius and one database's limits. ECS Fargate instead of EKS: there's one container image and one service here, with no service mesh or complex multi-service traffic needs, so Kubernetes would add real operational overhead (cluster upgrades, node or Fargate-profile management, extra add-ons just to reach Secrets Manager and IAM the way ECS reaches them natively) without solving a problem this system actually has. The cost is that if this ever needs to run alongside a larger fleet of services already standardized on Kubernetes, or needs multi-cloud portability, moving off ECS later is real migration work, not a config change. Row locking for reservations instead of optimistic locking or Redis: correctness comes first. The cost is that a very popular item forces buyers to queue on that one row. A single local transaction for reserve-and-create, and from `AWAITING_PAYMENT` onward, correctness comes from an idempotent `reconcile()` and a guard-checked state machine rather than a saga: the cost is that this reasoning has to be understood, not just the mechanism itself, since it's less obviously "one uniform pattern" than a textbook saga diagram would be. Direct, in-process calls instead of an outbox table and a message queue for getting payment updates to the orders and inventory modules: the two background jobs are lightweight, and there's no real process boundary between them and the API today, so a queue would add a relay, a dead-letter queue, and event schemas to maintain, without solving a problem that exists yet. The cost is that this choice has to be actively revisited, not just scaled up, the moment any of these modules becomes a genuinely separate deployed service: at that point a direct method call stops being physically possible, and the queue stops being optional. Authorize-then-capture for card, instead of charging immediately: this avoids holding a customer's money before shipping. The cost is that Wero and Open Banking still have to charge immediately regardless, since Unzer doesn't support authorize for them. Always re-fetching from Unzer on a webhook, using only the `paymentId` rather than trusting the webhook's own `retrieveUrl`: the cost is one extra API call per event, but it buys a real, trustworthy result and protection against fake webhooks, without needing a signature that Unzer doesn't actually provide.
